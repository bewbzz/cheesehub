...
import { FarmInfo, FarmSocials, buildSetFarmProfileAction } from "@/lib/farm";
...
// AtomicHub icon component
function AtomicHubIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor">
...
export function EditFarmProfile({ farm, open, onClose, onProfileUpdated }: EditFarmProfileProps) {
  const { session, accountName } = useWax();
  const { toast } = useToast();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [socialsOpen, setSocialsOpen] = useState(false);

  // Form state - pre-populate with farm data
  const [description, setDescription] = useState(farm.profile?.description || farm.description || "");
  const [avatar, setAvatar] = useState(farm.profile?.avatar || farm.logo || "");
  const [coverImage, setCoverImage] = useState(farm.profile?.cover_image || "");
  const [socials, setSocials] = useState<FarmSocials>({
  ...
  const handleSocialChange = (key: keyof FarmSocials, value: string) => {
  ...
  const handleSubmit = async () => {
    if (!session || !accountName) {
      toast({
        ...
        description: "Please connect your wallet to update the farm profile",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `Successfully updated ${farm.farm_name} profile`,
      });
      ...
      toast({
        title: "Update Failed",
        description: error instanceof Error ? error.message : "Failed to update farm profile",
        variant: "destructive",
      });
    } finally {
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Enter an IPFS hash (starting with Qm or bafy)
            </p>
          </div>
          ...
          <Collapsible open={socialsOpen} onOpenChange={setSocialsOpen}>
            <CollapsibleTrigger asChild>
              <Button 
                variant="outline" 
                className="w-full justify-between border-primary/50 text-primary hover:bg-primary/10"
              >
            ...
            <CollapsibleContent className="space-y-3 pt-3">
              ...
              <div className="space-y-1">
                ...
                <Input
                  id="medium"
                  placeholder="e.g. medium.com/yourblog"
              ...
              <div className="space-y-1">
                <Label htmlFor="atomichub" className="flex items-center gap-2 text-sm">
                  <AtomicHubIcon className="h-4 w-4" />
                  AtomicHub
                </Label>
                <Input
                  id="atomichub"
                  placeholder="e.g. wax.atomichub.io/collection"
              ...
              <div className="space-y-1">
                ...
                <Input
                  id="waxdao"
                  placeholder="e.g. waxdao.io/u/yourprofile"
