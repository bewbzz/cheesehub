...
export function FarmCard({ farm, onSelect }: FarmCardProps) {
  ...
  // Format reward pools for display - show raw on-chain balance
  const formatRewardPool = (pool: { symbol: string; balance: string; precision?: number }) => {
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex gap-2 pt-2">
          <Button
            variant="outline"
            size="sm"
            className="flex-1 bg-[hsl(30_100%_50%)] text-black border-[hsl(30_100%_50%)] hover:bg-background hover:text-foreground hover:border-border"
            onClick={(e) => {
              e.stopPropagation();
              handleViewFarm();
            }}
