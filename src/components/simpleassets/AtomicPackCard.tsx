...
export function AtomicPackCard({ pack, session, accountName, onSuccess }: AtomicPackCardProps) {
  ...
  const handleOpen = useCallback(async () => {
    ...
    const assetId = pack.assetIds[0]; // open first available pack
  ...
  return (
    <>
      <Card className="bg-card border-border hover:border-primary/40 transition-colors">
        <CardContent className="p-4 flex flex-col items-center text-center space-y-2">
          ...
          <Button
            ...
            disabled={!session || isOpening || pack.count === 0}
            onClick={handleOpen}
          >
            {isOpening ? (
              <><Loader2 className="h-3 w-3 mr-1 animate-spin" /> Opening...</>
            ) : (
              pack.count > 1 ? 'Open Pack' : 'Open Pack'
            )}
          </Button>
        </CardContent>
      </Card>
      ...
      <AtomicPackRevealDialog
        ...
        packAssetId={openedAssetId}
        unpackContract={pack.unpackContract}
        expectedCards={pack.cardsPerPack}
