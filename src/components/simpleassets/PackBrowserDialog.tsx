        ...
        <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-foreground">
              {pack.label} — {localCount} pack{localCount !== 1 ? 's' : ''}
            </DialogTitle>
          </DialogHeader>
              ...
              return (
                <Card key={globalIdx} className="bg-card border-border hover:border-primary/40 transition-colors">
                  <CardContent className="p-3 flex flex-col items-center text-center space-y-2">
                    {packImage ? (
                      <img src={packImage} alt={pack.label} className="w-3/4 h-auto rounded mx-auto" />
                    ) : (
                    ...
                    <Button
                      ...
                      disabled={!session || openingIdx !== null || !unboxType}
