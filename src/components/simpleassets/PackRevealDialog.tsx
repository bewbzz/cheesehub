...
/** Map pack symbol to boxtype used in pendingnft.a */
...
/** Fetch pending NFT rows from gpk.topps pendingnft.a scoped to user */
...
export function PackRevealDialog({
  ...
  // Unified polling effect for real opens — polls pendingnft.a
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        // Find new unboxingids not in pre-open set, with done=0
        const newRows = rows.filter(
          (r) => r.done === 0 && !preOpenUnboxingIds.has(r.unboxingid)
        );

        // Group by unboxingid, take the one with most rows (latest)
        ...
        console.log('[pack-reveal] poll', newRows.length, 'pending rows,', grouped.size, 'new unboxings, elapsed', Math.round(elapsed / 1000), 's');

        // Update wait message
        if (elapsed > 60000) {
          setWaitMessage('Taking longer than usual... still waiting for oracle');
        } else if (elapsed > 30000) {
          setWaitMessage('Still waiting for the RNG oracle...');
        }

        // Find the matching unboxing
        let targetRows: PendingNftRow[] | null = null;
        let targetUnboxingId: number | null = null;

        for (const [uid, uRows] of grouped) {
          // Match by boxtype if possible
          if (boxtype && uRows[0]?.boxtype === boxtype && uRows.length >= expectedCount) {
            targetRows = uRows;
            targetUnboxingId = uid;
            break;
          }
        }

        // Fallback: take largest new group with enough rows
        if (!targetRows) {
          for (const [uid, uRows] of grouped) {
            if (uRows.length >= expectedCount) {
              targetRows = uRows;
              targetUnboxingId = uid;
              break;
            }
        ...
        if (targetRows && targetUnboxingId !== null) {
          ...
          const rowIds = sorted.map((r) => r.id);

          console.log('[pack-reveal] found', cards.length, 'cards for unboxing', targetUnboxingId);
          setNewCards(cards);
          setPendingRowIds(rowIds);
    ...
    const startDelay = setTimeout(() => {
      if (cancelled) return;
      console.log('[pack-reveal] polling pendingnft.a started');
      poll();
  ...
  // Transition to collect phase when all cards revealed (real mode only)
  useEffect(() => {
    if (phase === 'revealing' && revealedCount >= newCards.length && newCards.length > 0) {
      if (demoCards && demoCards.length > 0) return; // demo skips collect
  ...
  const handleCollect = useCallback(async () => {
    ...
    try {
      ...
      console.log('[pack-reveal] getcards success for unboxing', unboxingId);
      setPhase('done');
      onComplete();
    } catch (e) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v: boolean) => { if (!v) handleClose(); }}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto bg-card border-border">
        ...
        {phase === 'waiting' && (
          <div className="flex flex-col items-center justify-center py-12 space-y-6">
            ...
            <div className="text-center space-y-2">
              ...
              {waitMessage ? (
                <p className="text-xs text-muted-foreground/60">{waitMessage}</p>
              ) : (
        ...
        {/* Reveal / collect / collecting / done phases all show cards */}
          ...
          <div className="space-y-6 py-4">
            ...
            {phase === 'collect' && (
              <div className="flex flex-col items-center gap-3 pt-2">
                {collectError && (
                  <p className="text-xs text-destructive text-center">{collectError}</p>
                )}
                <Button
                ...
                <p className="text-xs text-muted-foreground text-center">
                  Sign a transaction to deliver these cards to your wallet
                </p>
              </div>
            )}
