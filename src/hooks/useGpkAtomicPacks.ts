...
/** Map template IDs to their unpack contracts and expected card counts */
const PACK_CONFIG: Record<string, { contract: string; cards: number }> = {
  '13778':  { contract: 'gpkcrashpack', cards: 5 },   // Crash Gordon
  '48479':  { contract: 'burnieunpack', cards: 2 },   // Bernventures
  '51437':  { contract: 'burnieunpack', cards: 5 },   // Mitten
  '53187':  { contract: 'burnieunpack', cards: 5 },   // GameStonk
  '59072':  { contract: 'burnieunpack', cards: 3 },   // Food Fight
  ...
  const fetchPacks = useCallback(async () => {
    ...
    try {
      ...
      for (const asset of allAssets) {
        ...
        if (!config) continue; // skip unknown pack templates
        const existing = grouped.get(tid);
        if (existing) {
          existing.assets.push(asset);
        } else {
          grouped.set(tid, { assets: [asset], template: asset.template });
        }
      }
      ...
      console.log(`[GpkAtomicPacks] Found ${result.length} pack types, ${allAssets.length} total packs`);
      setPacks(result);
    } catch (e) {
